    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="pb-28">
        <div className="container mx-auto px-4">
          ...
          {/* Heatmap Grid */}
          <div className="mb-8">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-accent" /> Compliance Heatmap</h2>
            ...
            <Card className="premium-card">
              ...
              <CardContent className="space-y-3">
                {riskPredictions.map((rp) => (
                  <div key={rp.month} className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="text-xs font-medium">{rp.month}</p>
                        <p className="text-[10px] text-muted-foreground">{rp.events}</p>
                      </div>
                      <Badge variant="outline" className={`text-[10px] ${
                        rp.risk >= 70 ? "text-red-400 border-red-500/30" :
                        rp.risk >= 40 ? "text-amber-400 border-amber-500/30" :
                        "text-emerald-400 border-emerald-500/30"
                      }`}>{rp.risk}% risk</Badge>
                    </div>
                    <div className="w-full h-2 rounded-full bg-muted/30 overflow-hidden">
                      <motion.div
                        className={`h-full rounded-full ${riskColor(rp.risk)}`}
                        initial={{ width: 0 }}
                        animate={{ width: `${rp.risk}%` }}
